  ...
  //  private Vector3 oldscale;
   ...
   // public GameObject targetObjectPower;
    ...
    void Update()
    {
        ...
        if (scalChk != 0)
        { 
        PowerTemp = PowerTemp + (scalChk * scaleInc);
            }
        ...
        {
            if (PowerTemp < minPower)
            {
                PowerTemp = minPower;
            }
        }

        Pwr.PowerG = PowerTemp;
     ...
     //   }
...

        {
            if (OOB.fired == false)
            {
                ...
                force = LScale * PowerTemp * GetComponent<Transform>().up;
                // Debug.Log(force); Debug.Log(newscale);
